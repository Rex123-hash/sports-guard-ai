...
function Register({ onRegistered, onNav }) {
  const [step, setStep] = useStateR('idle'); // idle | uploading | metadata | hashing | done
  const [drag, setDrag] = useStateR(false);
  const [meta, setMeta] = useStateR({
  ...
  const [hash, setHash] = useStateR('');
  const [progress, setProgress] = useStateR(0);

  function handleDrop(e) { e.preventDefault(); setDrag(false); startUpload(); }
  function startUpload() {
    setStep('uploading'); setProgress(0);
    let p = 0;
    const t = setInterval(() => {
      p += 8 + Math.random() * 14;
      if (p >= 100) { p = 100; clearInterval(t); setStep('metadata'); }
      setProgress(p);
    }, 90);
  }
  function commit() {
    setStep('hashing');
    setTimeout(() => {
      const h = Array.from({ length: 16 }, () => '0123456789abcdef'[Math.floor(Math.random() * 16)]).join('');
      setHash(h); setStep('done');
      onRegistered && onRegistered({ ...meta, phash: h });
    }, 1700);
  }
  function reset() { setStep('idle'); setHash(''); setProgress(0); }
        ...
        <div className="fade-up delay-2">
          ...
          {step === 'idle' && (
            <div
              ...
              <div className="mono mt-3" style={{ fontSize: 12, color: 'var(--ink-mute)', letterSpacing: '0.06em' }}>
                JPG · PNG · WebP · up to 25 MB · or click to browse
              </div>
          ...
          {step !== 'idle' && (
            <div className="frame">
              <div className="frame-img" style={{ aspectRatio: '4/3', position: 'relative' }}>
                ...
                {step === 'uploading' && (
                  <div style={{
                    ...
                    height: 4, background: 'rgba(0,0,0,0.25)', zIndex: 5
                  }}>
              ...
              <div className="frame-cap">
                <span>asset_{Math.floor(Math.random() * 9000 + 1000)}.jpg · 2.4 MB · 1920×1080</span>
                <span>{step === 'uploading' ? `UPLOADING · ${Math.round(progress)}%` : step === 'hashing' ? 'HASHING…' : 'READY'}</span>
              </div>
            </div>
          )}
          ...
          <div className="card">
              ...
              <div className="grid grid-2 gap-4">
                <div className="field">
                  ...
                  <select className="select" value={meta.owner} onChange={e => setMeta({ ...meta, owner: e.target.value })}>
                    {window.SG_DATA.RIGHTS_HOLDERS.map(r => <option key={r}>{r}</option>)}
                  </select>
                </div>
                <div className="field">
                  ...
                  <select className="select" value={meta.sport} onChange={e => setMeta({ ...meta, sport: e.target.value })}>
                    {window.SG_DATA.SPORTS.map(s => <option key={s.id} value={s.id}>{s.label}</option>)}
                  </select>
                </div>
              </div>
              <div className="field">
            ...
            <div style={{ borderTop: '1px solid var(--line)', padding: '14px 22px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span className="mono" style={{ fontSize: 10.5, color: 'var(--ink-mute)' }}>
                Filed under: {meta.owner.toLowerCase().replace(/\s+/g, '-')}/{meta.sport}/
              </span>
              ...
              <div className="card" style={{ background: 'linear-gradient(135deg, var(--moss) 0%, #4A8E63 100%)', color: '#fff', borderColor: '#4A8E63' }}>
                <div className="card-pad-lg" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
                  <div>
                    ...
                    <div className="serif" style={{ fontSize: 26, lineHeight: 1.1, marginTop: 6, fontWeight: 400 }}>
                      Your frame is now under watch.
                    </div>
                    <div className="mono mt-2" style={{ fontSize: 11, opacity: 0.85 }}>
                      Continuously matched against any URL submitted to SportsGuard
                    </div>
                  </div>
