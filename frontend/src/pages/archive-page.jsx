          ...
          <div key={a.id} className="det-row" style={{ gridTemplateColumns: '60px 1.15fr 0.7fr 1fr 1fr 0.8fr 1.15fr' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'flex-start' }}>
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                ...
                <span className="tag solid-moss">{Icon.shield}VERIFIED</span>
              </div>
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                <button className="btn ghost" style={{ padding: '6px 10px', fontSize: 11, border: '1px solid var(--line)' }} onClick={() => setSelectedAsset(a)}>{Icon.doc}Preview certificate</button>
                <button className="btn ghost" style={{ padding: '6px 10px', fontSize: 11, border: '1px solid var(--line)' }} onClick={() => downloadCertificate(a)}>{Icon.copy}Export</button>
              </div>
            </div>
          </div>
        ))}
...
export function Drawer({ detection, assets, onClose }) {
  ...
  const dmcaNotice = buildDmcaNotice(detection, asset);

  function exportEvidence() { downloadHtmlFile(`sportsguard-evidence-${detection.id || 'report'}.html`, buildEvidenceReportHtml(detection, asset)); }
  function exportDmca() { downloadTextFile(`sportsguard-dmca-${detection.id || 'notice'}.txt`, dmcaNotice); }
    ...
    <>
      ...
      <div className="drawer">
        ...
        <div style={{ padding: 26 }}>
          ...
          <div className="card mt-6">
            <div className="card-head"><h3>Evidence report</h3></div>
            <div className="card-pad-lg">
              <EvidenceReportDocument detection={detection} asset={asset} />
              <div style={{ display: 'flex', gap: 8, marginTop: 16, flexWrap: 'wrap' }}>
                <button className="btn" onClick={() => copyTextToClipboard(report)}>{Icon.copy}Copy report</button>
                <button className="btn primary" onClick={exportEvidence}>{Icon.doc}Download report</button>
              </div>
            </div>
          </div>
